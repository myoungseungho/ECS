...
namespace Network
{
    public static class PacketBuilder
    {
        ...
        public static byte[] GuildDisband() { return Build(MsgType.GUILD_DISBAND); }
        ...
        /// <summary>GUILD_ACCEPT 빌드: guild_id(4)</summary>
        public static byte[] GuildAccept(uint guildId)
        {
            return Build(MsgType.GUILD_ACCEPT, BitConverter.GetBytes(guildId));
        }

        /// <summary>GUILD_LEAVE 빌드: empty</summary>
        public static byte[] GuildLeave() { return Build(MsgType.GUILD_LEAVE); }
        ...
        public static byte[] GuildInfoReq() { return Build(MsgType.GUILD_INFO_REQ); }

        /// <summary>GUILD_LIST_REQ 빌드: empty</summary>
        public static byte[] GuildListReq() { return Build(MsgType.GUILD_LIST_REQ); }

        /// <summary>GUILD_INFO 파싱: result(1) guild_id(4) guild_name(32) master_id(8) member_count(1) level(1) = 47B</summary>
        public static GuildInfoData ParseGuildInfo(byte[] payload)
        {
            var d = new GuildInfoData();
            d.Result = (GuildResult)payload[0];
            d.GuildId = BitConverter.ToUInt32(payload, 1);
            int nameEnd = 5;
            while (nameEnd < 5 + 32 && nameEnd < payload.Length && payload[nameEnd] != 0) nameEnd++;
            d.GuildName = Encoding.UTF8.GetString(payload, 5, nameEnd - 5);
            d.MasterId = BitConverter.ToUInt64(payload, 37);
            d.MemberCount = payload[45];
            d.Level = payload[46];
            return d;
        }

        /// <summary>GUILD_INVITE S→C 파싱: guild_id(4) guild_name(32) inviter_entity(8) = 44B</summary>
        public static GuildInviteData ParseGuildInvite(byte[] payload)
        {
            var d = new GuildInviteData();
            d.GuildId = BitConverter.ToUInt32(payload, 0);
            int nameEnd = 4;
            while (nameEnd < 4 + 32 && nameEnd < payload.Length && payload[nameEnd] != 0) nameEnd++;
            d.GuildName = Encoding.UTF8.GetString(payload, 4, nameEnd - 4);
            d.InviterEntity = BitConverter.ToUInt64(payload, 36);
            return d;
        }

        /// <summary>GUILD_LIST 파싱: count(1) {guild_id(4) guild_name(32) member_count(1) level(1)}*N = 38B/entry</summary>
        public static GuildListEntry[] ParseGuildList(byte[] payload)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                int nameEnd = off;
                while (nameEnd < off + 32 && nameEnd < payload.Length && payload[nameEnd] != 0) nameEnd++;
                g.GuildName = Encoding.UTF8.GetString(payload, off, nameEnd - off);
                off += 32;
                g.MemberCount = payload[off]; off += 1;
                g.Level = payload[off]; off += 1;
                guilds[i] = g;
            }
            return guilds;
        }
        ...
        /// <summary>TRADE_ACCEPT 빌드: requester_entity(8)</summary>
        public static byte[] TradeAccept(ulong requesterEntity)
        {
            return Build(MsgType.TRADE_ACCEPT, BitConverter.GetBytes(requesterEntity));
        }

        /// <summary>TRADE_DECLINE 빌드: empty</summary>
        public static byte[] TradeDecline() { return Build(MsgType.TRADE_DECLINE); }

        /// <summary>TRADE_ADD_ITEM 빌드: slot_index(1) count(2) = 3B</summary>
        public static byte[] TradeAddItem(byte slotIndex, ushort count)
        {
            byte[] payload = new byte[3];
            payload[0] = slotIndex;
            BitConverter.GetBytes(count).CopyTo(payload, 1);
            return Build(MsgType.TRADE_ADD_ITEM, payload);
        }

        /// <summary>TRADE_ADD_GOLD 빌드: amount(4)</summary>
        ...
        public static byte[] TradeConfirm() { return Build(MsgType.TRADE_CONFIRM); }

        /// <summary>TRADE_CANCEL 빌드: empty</summary>
        public static byte[] TradeCancel() { return Build(MsgType.TRADE_CANCEL); }

        /// <summary>TRADE_REQUEST S→C 파싱: requester_entity(8) requester_name(32) = 40B</summary>
        public static TradeRequestData ParseTradeRequest(byte[] payload)
        {
            var d = new TradeRequestData();
            d.RequesterEntity = BitConverter.ToUInt64(payload, 0);
            int nameEnd = 8;
            while (nameEnd < 8 + 32 && nameEnd < payload.Length && payload[nameEnd] != 0) nameEnd++;
            d.RequesterName = Encoding.UTF8.GetString(payload, 8, nameEnd - 8);
            return d;
        }

        /// <summary>TRADE_ADD_ITEM S→C 파싱(broadcast): slot_index(1) item_id(4) count(2) = 7B</summary>
        public static TradeAddItemData ParseTradeAddItem(byte[] payload)
        {
            var d = new TradeAddItemData();
            d.SlotIndex = payload[0];
            d.ItemId = BitConverter.ToUInt32(payload, 1);
            d.Count = BitConverter.ToUInt16(payload, 5);
            return d;
        }

        /// <summary>TRADE_RESULT 파싱: result(1)</summary>
        public static TradeResult ParseTradeResult(byte[] payload)
        {
            return (TradeResult)payload[0];
        }

        // ━━━ T031: 우편 ━━━

        /// <summary>MAIL_SEND 빌드</summary>
        public static byte[] MailSend(string recipient, string subject, string body, uint gold, uint itemId, ushort itemCount)
        {
            byte[] recipBytes = Encoding.UTF8.GetBytes(recipient);
            byte[] subjBytes = Encoding.UTF8.GetBytes(subject);
            byte[] bodyBytes = Encoding.UTF8.GetBytes(body);
            int len = 1 + recipBytes.Length + 1 + subjBytes.Length + 2 + bodyBytes.Length + 4 + 4 + 2;
            ...
            payload[off] = (byte)subjBytes.Length; off += 1;
            Buffer.BlockCopy(subjBytes, 0, payload, off, subjBytes.Length); off += subjBytes.Length;
        ...
        public static byte[] MailListReq() { return Build(MsgType.MAIL_LIST_REQ); }
        ...
        /// <summary>MAIL_LIST 파싱: count(1) {mail_id(4) sender_name(32) subject(64) read(1) has_attachment(1) sent_time(4)}*N = 106B/entry</summary>
        public static MailListEntry[] ParseMailList(byte[] payload)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                int nameEnd = off;
                while (nameEnd < off + 32 && nameEnd < payload.Length && payload[nameEnd] != 0) nameEnd++;
                m.SenderName = Encoding.UTF8.GetString(payload, off, nameEnd - off);
                off += 32;
                int subjEnd = off;
                while (subjEnd < off + 64 && subjEnd < payload.Length && payload[subjEnd] != 0) subjEnd++;
                m.Subject = Encoding.UTF8.GetString(payload, off, subjEnd - off);
                off += 64;
                m.IsRead = payload[off] == 1; off += 1;
                m.HasAttachment = payload[off] == 1; off += 1;
                m.SentTime = BitConverter.ToUInt32(payload, off); off += 4;
                mails[i] = m;
            }
            return mails;
        }

        /// <summary>MAIL_READ_RESP 파싱</summary>
        public static MailReadRespData ParseMailReadResp(byte[] payload)
        {
            var d = new MailReadRespData();
            int off = 0;
            d.Result = (MailReadResult)payload[off]; off += 1;
            d.MailId = BitConverter.ToUInt32(payload, off); off += 4;
            if (d.Result != MailReadResult.SUCCESS) return d;
            int nameEnd = off;
            while (nameEnd < off + 32 && nameEnd < payload.Length && payload[nameEnd] != 0) nameEnd++;
            d.SenderName = Encoding.UTF8.GetString(payload, off, nameEnd - off);
            off += 32;
            int subjEnd = off;
            while (subjEnd < off + 64 && subjEnd < payload.Length && payload[subjEnd] != 0) subjEnd++;
            d.Subject = Encoding.UTF8.GetString(payload, off, subjEnd - off);
            off += 64;
            ushort bodyLen = BitConverter.ToUInt16(payload, off); off += 2;
            d.Body = bodyLen > 0 ? Encoding.UTF8.GetString(payload, off, bodyLen) : "";
            off += bodyLen;
        ...
        /// <summary>MAIL_CLAIM_RESULT 파싱: result(1) mail_id(4) gold(4) item_id(4) item_count(2) = 15B</summary>
        public static MailClaimResultData ParseMailClaimResult(byte[] payload)
        {
            ...
            d.Gold = BitConverter.ToUInt32(payload, 5);
            d.ItemId = BitConverter.ToUInt32(payload, 9);
            d.ItemCount = BitConverter.ToUInt16(payload, 13);
            return d;
        }
